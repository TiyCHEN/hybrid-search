...
#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
    ...
    // 1/2
    hnsw[1] = std::move(std::make_unique<base_hnsw::RangeHierarchicalNSW<float>>(*hnsw[0]));
    // all 
#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
...
#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
    for (uint32_t i = 0; i < q2_indexes.size(); i++)  {
        ...
        file << q2_indexes[i] << " " << (back_up_l <= l && r <= back_up_r) << " " << tmp << " " << static_cast<int>(intersection.size()) << "\n";
    }
